...
test("transformRequestIn uses the CodeFree-O 1.7.0 client identity by default", async () => {
  const transformer = new SRDCloudTransformer({
    skipVersionUpdate: true,
    userId: "user-1"
  });
  ...
  assert.equal(result.config.headers["User-Agent"], "opencode/1.7.0");
  assert.equal(result.config.headers.clientVersion, "1.7.0");
});
